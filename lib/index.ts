...
export class SessionOperations {
	...
	private _prepareSession(encodingFile: {
	...
	public async run(
		...
		confidencePass = 0.6,
		inferenceOptions?: InferenceSession.RunOptions | undefined,
	): Promise<Array<SessionResult>> {
		...
		if (inputs.previouslyInstalled?.length) {
			input.set(
				"PreviouslyInstalled",
				new Set(
					inputs.previouslyInstalled.map((ext) => ext.toLowerCase()),
				),
			);
		}
		if (inputs.openedFileTypes?.length) {
			input.set(
				"OpenedFileTypes",
				new Set(
					inputs.openedFileTypes.map((ext) =>
						ext.toLowerCase().replace(/^(\w)/, ".$1"),
					),
				),
			);
		}
		if (inputs.activatedExtensions?.length) {
			input.set(
				"ActivatedExts",
				new Set(
					inputs.activatedExtensions.map((ext) => ext.toLowerCase()),
				),
			);
		}
		if (inputs.workspaceDependencies?.length) {
			input.set(
				...
				new Set(
					inputs.workspaceDependencies.map((ext) =>
						ext.toLowerCase(),
					),
				),
			);
		...
		if (inputs.workspaceFileTypes?.length) {
			input.set(
				"WorkspaceFileTypes",
				new Set(
					inputs.workspaceFileTypes.map((ext) =>
		...
		if (inputs.workspaceConfigTypes?.length) {
			input.set(
				...
				new Set(
					inputs.workspaceConfigTypes.map((ext) => ext.toLowerCase()),
				),
			);
		}
		...
		const result = await this._session!.run(
			...
			{
				[outputName]: new Tensor(outputTensor, [
					outputTensor.length,
					1,
				]),
			},
